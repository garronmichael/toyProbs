...
DocumentParser.prototype.parse = function()
{
  ...
  // if the chunk is an empty string
  if(chunk === '') {
    // we are at the end of the file and we return
    return;
  // otherwise
  } else {
    // split the chunk into individual chars
    var self = this;
    chunk.split('').forEach( function (v, i, a) {
      if (v === ' '){
        self.wordCount++;
        self.charCount++;
      } else if (v === '\\' && a[i + 1] === 'n'){
...
// Test.assertEquals(parser.charCount, 16);
// Test.assertEquals(parser.wordCount, 4);
// Test.assertEquals(parser.lineCount, 1);
